<template>
    ...
    <v-card>
        <v-card-title class="justify-center text-danger">
            <h4>Czy na pewno chcesz zgłosić {{getReportComponent.reportObject}}? </h4>
...
export default{
    ...
    methods:{
        ...
        async report(){
            const res = await this.callApi('post', this.getReportComponent.reportUrl, {targetIndex:this.getReportComponent.targetIndex,
                reason:this.reason, type:this.getReportComponent.reportObject});
            if(res.status ===201){
